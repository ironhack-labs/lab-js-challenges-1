...
function uniquifyArray(wordArray) {
  ...
  if (wordArray.length > 1) {
    ...
    while (index < wordArray.length - 1) {
      currentElement = wordArray[index];
      // Keep removing elements till the current element is not present in the rest of the array
      while (wordArray.includes(currentElement, index + 1)) {
        removeIndex = wordArray.indexOf(currentElement, index + 1);
  ...
  for (let row = 0; row < matrix.length; row++) {
    for (let column = 0; column < matrix[row].length; column++) {
      ...
      if (row + N < matrix.length) {
        ...
        if (maxMult === null || maxMult < rowMult) {
          maxMult = rowMult;
        }
      }
