...
function howManyTimes(arraysWord, word) {
  let count = 0;
  for (let i = 0; i < arraysWord.length; i++) {
    if (arraysWord[i] === word) {
      count++;
    }
  }
  return count;
...
function uniquifyArray(duplicateArray) {
  ...
  for (let i = 0; i < duplicateArray.length; i++) {
    if (array.indexOf(duplicateArray[i]) === -1) {
      array.push(duplicateArray[i]);
    }
  }
  return array;
}
