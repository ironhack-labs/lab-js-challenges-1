...
function filterOut(originalArray, wordsToRemove) {
  ...
  const answer = originalArray.filter((element) => {
    return !wordsToRemove.includes(element);
  });
...
function greatestProduct(matrix) {
  ...
  for (let i = 0; i <= matrix.length - 4; i++) {
    for (let j = 0; j <= matrix[i].length - 4; j++) {
      ...
      if (currentMulRow > biggestOne) {
        biggestOne = currentMulRow;
      }
      if (currentMulCol > biggestOne) {
        biggestOne = currentMulCol;
      }
    }
  }
